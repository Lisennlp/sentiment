...
class InputExample(object):
    ...
    def __init__(self, id, text_a, passage, label=None):
        self.id = id
...
class DataProcessor(object):
        ...
        with open(path, 'r', encoding='utf-8') as reader:
            for index, line in enumerate(reader):
                ...
                if not passage or passage == 'nan':
                    continue
                label = int(line[0][-1])
                example = InputExample(id=index, text_a=text_a, passage=passage, label=label)
    ...
    def get_eval_examples(self, path):
        ...
        with open(path, 'r', encoding='utf-8') as reader:
            for index, line in enumerate(reader):
                ...
                example = InputExample(id=id, text_a=text_a, passage=passage, label=id)
                examples.append(example)
    ...
    def get_predict_examples(self, path):
        ...
        for id, passage, t, p, v in zip(datas['微博id'], datas['微博中文内容'], datas['微博发布时间'], datas['微博图片'], datas['微博视频']):
            p_mes, v_mes = '无', '无'
            if p != '[]':
                p_mes = '有'
            if v != '[]':
                v_mes = '有'
            text_a = f"{t} {p_mes} {v_mes}"
            if not passage or not isinstance(passage, str):
                continue
            passage = self.data_deal(passage)
...
def convert_examples_to_features(args, examples, max_seq_length, tokenizer):
    ...
    for (ex_index, example) in enumerate(examples):
        # try:
        query_tokens = ["[CLS]", *tokenizer.tokenize(example.text_a), "[SEP]"]
        segments_ids = [0] * len(query_tokens)

        passage_tokens = [*tokenizer.tokenize(example.passage)[:max_seq_length], "[SEP]"]
        segments_ids.extend([1] * len(passage_tokens))

        input_ids = tokenizer.convert_tokens_to_ids([*query_tokens, *passage_tokens])
        ...
        # print(f'inlen = {len(segments_ids)}, len(input_ids) = {len(input_ids)} len(mask_ids) = {len(mask_ids)}')
   ...
   # bert_config = BertConfig.from_json_file(vocab_path)
    ...
    def write_predict_file(model, loop, data_loader, file_path):
        ...
        if not args.do_eval:
            ...
            data_df = pd.DataFrame({'id': ids, 'pred': logits, 'real': labels, 'probs': probs, 'match': match_list, 'passage': passages})
