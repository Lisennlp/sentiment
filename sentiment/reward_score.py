...
import tokenization
from modeling import BertForSequenceClassification, BertForRewardScore
    ...
    def get_examples(self, path):
        ...
        with open(path, 'r') as f:
            for index, line in enumerate(f):
                ...
                # label = [0, 0, 0]
                # label[int(line['chosen'])] = 1
                text = line['sents']
                text = text.strip()
                example = InputExample(id=index, summary=line['summaries'], passage=text, label=int(line['chosen']))
...
def convert_examples_to_features(examples, max_seq_length, tokenizer, type_='train'):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        passage_tokens = ["[CLS]"] + tokenizer.tokenize(example.passage)
        passage_ids = tokenizer.convert_tokens_to_ids(passage_tokens)
        inputs_ids, segments_ids, masks_ids = [], [], []
        for summary in example.summary:
            summary_tokens = tokenizer.tokenize(summary)
            summary_ids = tokenizer.convert_tokens_to_ids(summary_tokens)
            cut_passage_len = max_seq_length - len(summary_ids)
            input_ids = passage_ids[:cut_passage_len-1] + [102] + summary_ids
            segment_ids = [0] * len(passage_ids[:cut_passage_len - 1])  + [0] + [1] * len(summary_ids)
            ...
            assert len(segment_ids) == len(input_ids) == len(mask_ids)
            inputs_ids.append(input_ids)
            segments_ids.append(segment_ids)
            masks_ids.append(mask_ids)
        features.append(
            InputFeatures(input_ids=inputs_ids,
                        input_mask=masks_ids,
    ...
    pickle.dump(features, open(f'/nas/lishengping/temp/reward_score_features_cache.{type_}.pkl', 'wb'))
    return features
...
def main():
    ...
    def evaluating(model, eval_dataloader):
        ...
        for step, batch in enumerate(eval_dataloader):
            ...
            pred = torch.argmax(logits.view(-1, 3), dim=-1)
            right = (pred == label_ids.view(-1)).sum()
            total_right += right.item()
            total = (step + 1) * args.eval_batch_size
            acc = total_right / total
            if (step + 1) % 20 == 0:
                print(f'eval acc: {acc} eval loss: {eval_loss.item() / step}')
        model.train()
        return eval_loss.item() / step

    def eval_meric(model, data_loader):
        eval_loss = evaluating(model, data_loader)
    ...
    cache_dev_file = '/nas/lishengping/temp/reward_score_features_cache.eval.pkl'
    cache_train_file = '/nas/lishengping/temp/reward_score_features_cache.train.pkl'
    ...
    model = BertForRewardScore.from_pretrained(args.bert_model, num_choices=1)
    ...
    if args.do_train:
        ...
        for epoch in range(int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(train_loader):
                ...
                right = right.item()
                total_right += right
                # print(pred.tolist(), label_ids.tolist())
                total = (step + 1) * args.train_batch_size
                acc = total_right / total
                if step % 20 == 0:
                    logging.info(f'epoch {epoch} step {step}/{len(train_loader)} loss = {loss} total acc = {acc} single acc: {right / args.train_batch_size}')
